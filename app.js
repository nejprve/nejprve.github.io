(function(){
  ...
  var fileHandle = null; // nelze bezpečně serializovat, držíme jen v paměti po dobu relace
  ...
  async function pickExportFile(){
    ...
    try{
      ...
      if (perm !== 'granted'){
        notifyHint('Nedostatečné oprávnění k zápisu do souboru.', true);
        fileHandle = null;
        return;
      }
      notifyHint('Cílový soubor vybrán. Auto‑export připraven.');
    }catch(err){
      notifyHint('Výběr souboru zrušen.', true);
    }
  }
  ...
  async function writeFile(blob){
    ...
    try{
      ...
      notifyHint('Auto‑export selhal: ' + err.message, true);
    }
  }
  ...
  function debounceAutoExport(run){
    clearTimeout(autoExportTimer);
    autoExportTimer = setTimeout(run, 800);
  }
  ...
  function setAutoExportUI(){
    ...
    if(pickBtn){ pickBtn.disabled = !supportsFS; }
    if(!supportsFS){
      notifyHint('Auto‑export vyžaduje Chrome/Edge (File System Access API).', true);
    }
  }
  ...
  var $$ = function(s, rootEl){ return (rootEl||document).querySelector(s) };

  var todoList = $$('#todoList', root), doneList = $$('#doneList', root);
  var input = $$('#newText', root), addBtn = $$('#addBtn', root), emptyMsg = $$('#emptyMsg', root);
  var exportBtn = $$('#exportBtn'), importFile = $$('#importFile');
  var newTag = $$('#newTag', root), filterTag = $$('#filterTag', root);

  // dialog prvky
  var dlg = $('#timeDialog');
  var dlgCreated = $('#dlgCreated');
  var dlgCompleted = $('#dlgCompleted');
  var dlgSave = $('#dlgSave');
  ...
  function confirmDelete(message){
    return new Promise(function(resolve){
      ...
      var onClose = function(){
        ...
        resolve(confirmDlg.returnValue === 'yes');
      };
  ...
  function applyTagFilter(arr){
    if(currentFilter === 'all') return arr.slice();
    return arr.filter(function(n){ return n.tag === currentFilter });
  }

  function isFutureNote(note){
    var now = Date.now();
    return (typeof note.created === 'number' && note.created > now) ||
           (typeof note.completed === 'number' && note.completed > now);
  }

  function render(){
    var filtered = applyTagFilter(state);

    var todos = filtered.filter(function(n){ return !n.done })
                        .sort(function(a,b){ return b.created - a.created });
    var dones = filtered.filter(function(n){ return n.done })
                        .sort(function(a,b){
    ...
    renderList(todoList, todos, false);
    renderList(doneList, dones, true);
  ...
  function tagLabel(tag){
    if(tag==='skola') return 'Škola';
    if(tag==='domov') return 'Domov';
    if(tag==='seberozvoj') return 'Seberozvoj';
    return tag;
  }

  function renderList(container, items, doneSection){
    container.innerHTML = '';
    items.forEach(function(note){
      ...
      cb.addEventListener('change', function(){
        ...
        if (note.done && !note.completed) {
          note.completed = Date.now();
        }
        if (!note.done) {
          note.completed = null;
        }
      ...
      var m = '(vložené ' + fmt(note.created) + (note.completed ? ', splněno ' + fmt(note.completed) : '') + ')';
      ...
      editTimeBtn.className = 'iconbtn';
      editTimeBtn.type = 'button';
      editTimeBtn.title = 'Upravit časy';
      editTimeBtn.textContent = '🕒';
      editTimeBtn.addEventListener('click', function(){
        dlgEditingId = note.id;
        dlgCreated.value = toLocalDatetimeValue(note.created);
        dlgCompleted.value = note.completed ? toLocalDatetimeValue(note.completed) : '';
      ...
      if (future) li.appendChild(futureSpan);
  ...
  function exportNotes(){
    ...
    var blob = new Blob([JSON.stringify(payload,null,2)], {type:'application/json'});
    if (fileHandle && autoExportEnabled){
      writeFile(blob);
      return;
    }
  ...
  function importNotes(file){
    ...
    reader.onload = function(){
      try{
        ...
        if(!data) throw new Error('Soubor je prázdný.');
        var arr = Array.isArray(data) ? data : data.notes;
        if(!Array.isArray(arr)) throw new Error('Neočekávaný formát. Čekám pole \"notes\" nebo přímo pole poznámek.');
        ...
        state.forEach(function(n){
          byId[n.id] = n;
          sigSet[n.text + '||' + n.created + '||' + n.tag] = true;
        });
  ...
  function validateTimes(createdVal, completedVal){
    ...
    if (future){
      if(dlgInfo){ dlgInfo.hidden = false; }
    } else {
      if(dlgInfo){ dlgInfo.hidden = true; }
    }
  ...
  var autoToggle = document.getElementById('autoExportToggle');
  var pickBtn = document.getElementById('pickFileBtn');
  setAutoExportUI();
  if (autoToggle){
    autoToggle.addEventListener('change', function(){
      autoExportEnabled = autoToggle.checked;
      localStorage.setItem(LS_KEY + '_autoExport', JSON.stringify(!!autoExportEnabled));
      if (autoExportEnabled && !fileHandle){
        pickExportFile();
      } else {
        notifyHint(autoExportEnabled ? 'Auto‑export zapnut.' : 'Auto‑export vypnut.');
      }
    });
  }
  if (pickBtn){
    pickBtn.addEventListener('click', pickExportFile);
  }
